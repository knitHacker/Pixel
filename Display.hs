...
squares n w = squares' xPoints yPoints
    ...
    where subSquareW = w / (fromIntegral n)
          xPoints = subPoints (-(w/2)) 0
          yPoints = subPoints (-(w/2)) 0
          subPoints subW mult = let newSub = subW + (subSquareW * mult) in
                if newSub <= (w/2) then newSub : (subPoints subW (mult + 1))
                                   else []
...
display :: IORef (Maybe (Either Coord2 Coord2)) -> IORef Int -> DisplayCallback
display pos width = do
    w <- get width
    let subSquares = squares w 1.0 in do
